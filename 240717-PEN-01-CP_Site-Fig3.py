...
#
# Written on: May 26, 2023 | Updated: April 14, 2024
# ----
### Notes
# April 14, 2024 Updated with new CP data finalized
# April 14, 2024: Focus on 18 ka comparison
...
#---- Data ----#
dataset = loadmat('/Users/kaustubh/Documents/MATLAB/Scripts/Projects/02-PaleoENSO/20240414-PaleoENSO_Data.mat', squeeze_me=True)
cp_data_lh = dataset['CP_Costa_LH'][:,1]
cp_data_lgm = dataset['CP_Costa_LGM'][:,1]
#---- Model ----#
...
cp_pi_t = sites_data[0]['site'][8]['tos']
cp_lgm_t = sites_data[3]['site'][8]['tos']
cp_pi_s = sites_data[0]['site'][8]['sos']
cp_lgm_s = sites_data[3]['site'][8]['sos']

#---- Forward Model ----#
cp_pi_sw = cp_pi_s*0.27 - 8.88
cp_lgm_sw = cp_lgm_s*0.27 - 8.88

cp_pi = (cp_pi_sw-0.27) + (14.9-cp_pi_t)/4.8
cp_lgm = (cp_lgm_sw-0.27) + (14.9-cp_lgm_t)/4.8

# %% Foram Picking Cell
# Calculate standard deviations from picked populations across PI, PI-2, and LGM

num = cp_data_lgm.size
mc = 10000
...
#---- Save figure ----#
plt.savefig('20240414-PEN-Fig-a-CP-Hist.pdf', format='pdf', bbox_inches='tight')
...
sns.set(style='darkgrid',palette='muted',font='Helvetica Neue',font_scale=1)
[fig2,ax] = plt.subplots(1,1,figsize=(6,5))

# Plot Q-Q Envelope
ax.fill_between(np.median(q_pi,axis=1), np.percentile(q_lgm,10,axis=1), np.percentile(q_lgm,90,axis=1), 
                alpha=0.4,color='xkcd:sea blue',label='PI vs Full LGM ENSO')
ax.fill_between(np.median(q_pi,axis=1), np.percentile(q_alt,12,axis=1), np.percentile(q_alt,88,axis=1), 
                alpha=0.4,color='xkcd:Goldenrod',label='PI vs PI-Anom+LGM-AnnCyc')

a=0.7 # For dashed line
ax.plot([-a, a], [-a, a],':',color='xkcd:dark grey')

# Plot IFA data
q_cp_lh = np.quantile(cp_data_lh-np.mean(cp_data_lh),np.linspace(0,1,cp_data_lgm.size//2),axis=0)
q_cp_lgm = np.quantile(cp_data_lgm-np.mean(cp_data_lgm),np.linspace(0,1,cp_data_lgm.size//2),axis=0)

slope, intercept, r_value, p_value, std_err = stats.linregress(q_cp_lh,q_cp_lgm)
...
ax.plot(q_cp_lh,q_cp_lgm,marker='o',
        ...
        markerfacecolor='xkcd:Salmon',markeredgecolor='xkcd:White',label='New IFA @ CP Site')

# Graph Specifications
plt.xlabel('Late Holocene IFA-$\delta^{18}$O')
plt.ylabel('LGM IFA-$\delta^{18}$O')
plt.grid(linestyle='--')
plt.yticks(np.arange(-a,a+0.1*a,0.2))
plt.xticks(np.arange(-a,a+0.1*a,0.2))
plt.gca().invert_yaxis()
plt.gca().invert_xaxis()
plt.legend()

#---- Save figure ----#
fig2.savefig('20240414-PEN-Fig-b-CP-QQ.pdf')
...
df2 = pd.DataFrame({'LH': cp_data_lh-np.median(cp_data_lh),'LGM': cp_data_lgm-np.median(cp_data_lgm)})
labels = {'LH','LGM'}
...
sns.histplot(
   ...
   alpha=.5, linewidth=0,kde=True,bins=np.arange(-1.1,1.1,0.2),legend=True)
...
#---- Save figure ----#
plt.savefig('20240510-PEN-Data_Hist.pdf', format='pdf', bbox_inches='tight')
...
# axx.set_ylim([-5,5])
axx.invert_yaxis()
plt.savefig('20240414-PEN-Data_Swarm.pdf', format='pdf', bbox_inches='tight')
plt.show()